/**
 * /bets/online-bets — Section 4 page.
 *
 * Lists all bets placed through the User Panel (channel = 'online'),
 * showing the row shape required by the spec (Full Name, Phone, Stake,
 * Won, Bonus Used, Net Win, Bet ID, Paid, Status, Payment Type,
 * Paid Amount, Paid At, Date).
 *
 * Actions:
 *   - View Slip (modal with selections, hashes and cashout toggle)
...
interface OnlineBetRow {
  ...
  paidAt: string;
  date: string;
  /** raw row for slip detail look-up */
...
function toRow(b: betsApi.AdminBet): OnlineBetRow {
  const stake = num(b.stake);
  const payout = num(b.actual_payout);
  const potential = num(b.potential_payout);
  ...
  return {
    ...
    paymentType: String(
      (b.metadata?.payment_type as string | undefined) ?? 'Regular'
    ),
    ...
    <div className="bg-white rounded-lg w-full max-w-6xl max-h-[90vh] overflow-y-auto">
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Slip Details</h2>
          <button
            onClick={onClose}
...
const SlipBody = ({ bet }: { bet: betsApi.AdminBetDetail }) => {
  ...
  const cashoutAvailable = Boolean(bet.metadata?.cashout_available ?? false);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        />
        <Field
          label="Cashout Available"
          value={cashoutAvailable ? 'Yes' : 'No'}
        />
...
export function OnlineBets() {
  ...
  const [rows, setRows] = useState<OnlineBetRow[]>([]);
  ...
  useEffect(() => {
    ...
    betsApi
      ...
      .catch((err: Error) =>
        toast(`Failed to load online bets: ${err.message}`, 'error')
      )
      .finally(() => {
  ...
  const handleCancel = async (row: OnlineBetRow) => {
    ...
    if (
      !window.confirm(
        `Cancel bet ${row.id.slice(0, 8)}? The stake of ${row.stake.toFixed(
          2
        )} ${row.raw.currency} will be refunded to the user.`
      )
    ) {
      return;
    }
    ...
    try {
      await betsApi.cancelBet(row.id, 'Cancelled from admin online bets page');
  ...
  const handleExport = () => {
    ...
    downloadCsv(exportColumns, filteredData, `online-bets-${todayStamp()}`);
  ...
  const filters = [
    ...
    {
      label: 'Payment Type',
      options: ['Regular', 'Cashback', 'Cashout'],
  ...
  const columns = [
    ...
    { header: 'Date', accessor: 'date' as const },
    {
      ...
      render: (id: string, row: OnlineBetRow) => (
  ...
  return (
    <div className="space-y-6">
      ...
      {slip && (
        <SlipModal data={slip} onClose={() => setSlip(null)} />
      )}
    </div>
  );
}
